...
def p(gb):
    ...
    for key, pandas_vals in df.groupby(list_gb):
        ...
        vals_to_print = filter(lambda el: el[2] in ["no_rn"], pandas_vals.values)
        ...
        for res_num in range(len(results)):
            ...
            if res_to_compare_num > 0 and results[res_num][1] == "nonfav":
            ...
            # to_str = f"\cellcolor{{{colors[srv.index(results[res_num][:-1][1])]}!10}}{formatted_results['time']}"
            to_str = f"{formatted_results['dynamic']}"
